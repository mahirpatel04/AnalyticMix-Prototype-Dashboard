...
def convertBinaryFileToDataFrame(fileData):
    return pd.read_csv(StringIO(fileData.decode('utf-8')))

def getColumns(df):
    return [(columnName, columnName) for columnName in df.columns]
    
def process(df, independents, dependents, models):
    X = df[independents]
    y = df[dependents]
